...
##                                                                            ##
## Note: This script has been run on a different R version with different     ##
## versions of sf to test dependence on availability of s2.                   ##
################################################################################
# Clean up memory
rm(list = ls(all = TRUE))
################################################################################
...
#version_string <- "polygon-based_test_nos2"
version_string <- "polygon-based_test_s2"
...
for (type in seq_along(glwd_classes)) {
  ...
  if (length(type_index) > 0) {
    ...
    if (exists("sf_use_s2")) {
      ...
      ### The line below was uncommented vor version "polygon-based_test_nos2"
      # sf_use_s2(FALSE)
    }
