...
/* TODO nitrogen limitation of decomposition including variable decay rates Brovkin,
      fn_som=flux_soil[l].fast.carbon/CN_ratio_fast+flux_soil[l].slow.carbon/CN_ratio_slow;
        ...
        if(stand->type->landusetype==GRASSLAND)
          stand->cell->output.anmineralization_mgrass+=F_Nmineral*stand->frac;
#ifdef MICRO_HEATING
  ...
  if(gtemp_soil[0]>0)
  {
    for(p=0;p<soil->litter.n;p++)
    {
      ...
      forrootsoillayer(l)
      {
        ...
        {
          stand->cell->output.cshift_fast_nv[l]+=param.fastfrac*(1-param.atmfrac)*decom_sum.carbon*soil->c_shift_fast[l][soil->litter.item[p].pft->id];
          stand->cell->output.cshift_slow_nv[l]+=(1-param.fastfrac)*(1-param.atmfrac)*decom_sum.carbon*soil->c_shift_slow[l][soil->litter.item[p].pft->id];
        }
        ...
        if(N_sum>0) /* immobilization of N */
        {
          ...
          if(n_immo >0) 
          {
            ...
            if(stand->type->landusetype==GRASSLAND)
              stand->cell->output.animmobilization_mgrass+=n_immo*stand->frac;
