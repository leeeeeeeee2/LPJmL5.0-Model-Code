                 ...
                 )
{
  ...
  foreachstand(stand,s,cell->standlist)
  {
    ...
    {
      stand->cell->output.mgrass_n2o_nit+=hetres.nitrogen*stand->frac;
      stand->cell->output.mgrass_rh+=hetres.carbon;
    }
    ...
    if(config->with_nitrogen)
    {
      if(config->with_nitrogen==UNLIM_NITROGEN)
      {
        ...
        if(stand->type->landusetype==GRASSLAND)
          cell->output.andepo_mgrass+=2000*stand->frac;
      }
      else if(!config->no_ndeposition)
      {
        ...
        if(stand->type->landusetype==GRASSLAND)
          cell->output.andepo_mgrass+=(climate.nh4deposition+climate.no3deposition)*stand->frac;
      }
#ifdef DEBUG_N
    ...
    if(config->with_nitrogen)
    {
      ...
      if(stand->type->landusetype==GRASSLAND)
      {
        cell->output.mgrass_bnf+=bnf*stand->frac;
      }
    }
    ...
    if(config->with_nitrogen)
    {
      ...
      if(stand->type->landusetype==AGRICULTURE || stand->type->landusetype==SETASIDE_RF || stand->type->landusetype==SETASIDE_IR)
        ...
        cell->output.mgrass_volatilization+=nh3*stand->frac;
