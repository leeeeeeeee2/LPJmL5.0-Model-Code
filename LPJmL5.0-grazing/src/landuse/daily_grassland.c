                    ...
                    )            /** \return runoff (mm) */
{
  ...
  if (config->with_nitrogen && config->fix_fertilization){
    /* determine fertilization date */
    if (config->sdate_option==PRESCRIBED_SDATE){
      if (stand->cell->coord.lat > 30 || stand->cell->coord.lat < -30){
        fday = stand->cell->ml.sdate_fixed[2];  /* maize sowing date */
      } else {
        fday = stand->cell->ml.sdate_fixed[0];  /* wheat sowing date */
      }
    } else {
      if (stand->cell->coord.lat > 30 || stand->cell->coord.lat < -30){
        fday = stand->cell->ml.cropdates[2].spring_sdate20;  /* maize sowing date */
      } else {
        fday = stand->cell->ml.cropdates[0].spring_sdate20;  /* wheat sowing date */
      }
    }
    if (fday==0) fday=91;
    if (fday==day){
      fertil = param.fertilizer_rate;
      ...
      stand->cell->output.anfert_mgrass+=fertil*stand->frac;
    }
    /* do same for manure cell->ml.manure_nr[irrigation].crop[pft->par->id - npft]; */
  }
  /* green water inflow */
 ...
 */
    ...
    output->mgrass_gpp+=gpp;
    output->mgrass_ra+=gpp-npp;
    output->mgrass_agbmean+=agb_grass(pft);
  ...
  switch(stand->cell->ml.grass_scenario)
  {
    ...
    case GS_GRAZING_EXT: /* ext. grazing  */
      ...
      if (cleaf > STUBBLE_HEIGHT_GRAZING_EXT) /* minimum threshold */
      {
        ...
        /*
        if (year < config->firstyear-20) stand->cell->ml.nr_of_lsus_ext = 0.2;
        if (year >= config->firstyear-20 & year < config->firstyear)
          stand->cell->ml.nr_of_lsus_ext = (param.lsuha-0.2)/(20)*(year-config->firstyear+20)+0.2;
        if (year >= config->firstyear) stand->cell->ml.nr_of_lsus_ext = param.lsuha;
        */
      }
      break;
