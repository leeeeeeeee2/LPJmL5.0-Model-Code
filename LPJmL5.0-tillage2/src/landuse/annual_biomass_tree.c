                        ...
                        )                      /** \return stand has to be killed (TRUE/FALSE) */
{
  ...
  */
  ...
  if(isdead)
  {
    ...
    /* no tillage if IR and RF SETASIDE stands are converted in each other. Set from stand->cell->ml.with_tillage to FALSE*/
    if(setaside(stand->cell,stand,config->pftpar,FALSE,intercrop,npft,irrigation->irrigation,year))
      return TRUE;
  }
  else
