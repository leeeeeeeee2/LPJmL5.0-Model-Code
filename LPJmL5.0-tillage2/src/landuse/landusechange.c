             ...
             )
{
  ...
  if(s!=NOT_FOUND)
  {
    ...
    if(natstand->frac>minnatfrac)
    {
      ...
      if(!timberharvest)
      {
        /* no tillage if IR and RF SETASIDE stands are converted in each other */
        if(setaside(cell,getstand(cell->standlist,pos),pftpar,FALSE,intercrop,npft,irrig,year))
                              ...
                              )
{
  ...
  if(grassstand->frac<=difffrac+epsilon)
  {
    ...
    if(setaside(cell,getstand(cell->standlist,s),pftpar,TRUE,intercrop,npft,data->irrigation,max(param.till_startyear,year)))
      delstand(cell->standlist,s);
  }
  else
  {
    ...
    /* no tillage if IR and RF SETASIDE stands are converted in each other */
    if(setaside(cell,getstand(cell->standlist,pos),pftpar,FALSE,intercrop,npft,data->irrigation,year))
      delstand(cell->standlist,pos);
  }
  ...
  /* needs to be called before establishment, to ensure that regrowth is possible in the following year*/
{
  ...
  if(cell->ml.dam)
    landusechange_for_reservoir(cell,pftpar,npft,istimber,with_tillage,intercrop,ncft,year);
    ...
    if(s!=NOT_FOUND && s2!=NOT_FOUND)
    {
      ...
      if(difffrac2<0) /* move irrigated setaside to rainfed setaside */
      {
        ...
        else
        {
          ...
          /* no tillage if IR and RF SETASIDE stands are converted in each other */
          if(setaside(cell,getstand(cell->standlist,pos),pftpar,FALSE,intercrop,npft,FALSE,year))
            delstand(cell->standlist,pos);
      ...
      else /* move rainfed setaside to irrigated setaside */
      {
        ...
        else
        {
          ...
          /* no tillage if IR and RF SETASIDE stands are converted in each other */
          if(setaside(cell,getstand(cell->standlist,pos),pftpar,FALSE,intercrop,npft,TRUE,year))
             delstand(cell->standlist,pos);
          stand->frac-=movefrac;
        }
      }
  ...
  for(i=0;i<2;i++)
  {
    ...
    if(difffrac>=epsilon && cell->lakefrac+cell->ml.reservoirfrac+cell->ml.cropfrac_rf+cell->ml.cropfrac_ir<(1-epsilon)) 
      deforest(cell,difffrac,pftpar,with_tillage,intercrop,npft,FALSE,istimber,i,ncft,year,minnatfrac_luc);  /*deforestation*/
    ...
    if(s!=NOT_FOUND)
    {
      ...
      if(difffrac>epsilon)
        grasslandreduction(cell,difffrac,pftpar,with_tillage,intercrop,npft,s,stand,istimber,ncft,pft_output_scaled,year);
      else if(difffrac<-epsilon)
